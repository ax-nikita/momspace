...
if (!function_exists('momspace_theme_inline_style')):

	function momspace_theme_inline_style()
	{

		wp_enqueue_style('momspace-custom-style', get_template_directory_uri() . '/assets/css/custom-style.css');
		
		
		$theme_main_color = momspace_get_customize_option('theme_main_color');
		
		$theme_border_type = momspace_get_option('theme_border_type');


		$topbar_bg_img = momspace_get_option('topbar_bg_img');
		...
		$topbar_bg_img_color = momspace_get_option('topbar_bg_color');	
		...
		if (!empty($theme_main_color))
		{
			...
			$custom_css .= ' .blog-sidebar .widget_search form button, .cutsom-post-block-list-inner .recent-postlist-wrap-inner:hover .recent-post-number-wrap, .category-box.news-cat-boxone a, button.momspace-load-more-btn, .main-container .theme-pagination-style ul.page-numbers li span.current, .main-container .theme-pagination-style ul.page-numbers li a.page-numbers:hover {background-color: ' . esc_attr($theme_main_color) . '!important;}';
        ...
        if ($categories_widget_color) {
            foreach( $categories_widget_color as $tag) {
				...
				$title_bg_Color = get_term_meta($tag->term_id, 'momspace', true);
		...
		wp_add_inline_style('momspace-custom-style', $custom_css);
	}
	add_action('wp_enqueue_scripts', 'momspace_theme_inline_style');
